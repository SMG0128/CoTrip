...
/**
 * @param {object} deps
 * @param {{ analyze(input:{rawText:string,context?:object}): Promise<{text:string}>, coordinate?(input:object): Promise<{text:string}> }} deps.aiProvider
...
function createGateway({ aiProvider, secret, maxBodyBytes = 64 * 1024, maxRawTextLength = 1000 }) {
  async function handle({ method, url, headers = {}, bodyText = '' }) {
    ...
    if (method !== 'POST' || (url !== '/analyze' && url !== '/coordinate')) {

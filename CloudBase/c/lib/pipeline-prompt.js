...
const SNAPSHOT = [
  ...
  '每项只能包含 id（仅允许时）、type、title、time、locationRequirement、alternatives、transportPreference。',
  'type 只能是 SPORT、DINING、TRANSPORT、ENTERTAINMENT、OTHER。',
  'time 必须包含 start 与 timezone，必须有 end；start/end 必须是带 Z 或数值时区偏移的 ISO-8601，end 必须晚于 start。',
  'locationRequirement 只能表达 city、district、locationId、query；不得声称已验证真实地点。',
...
const PIPELINE_SYSTEM_PROMPTS = {
  ...
  TRIP_UPDATE: [
    ...
    '删除活动必须在 ui.removedEventIds 声明其稳定 ID；不得遗漏无关活动；换地点沿用目标 ID；调整时间必须同步 end 并保持其他活动身份。',

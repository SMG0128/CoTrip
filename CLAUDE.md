...
- `routeOptionService` → **Disabled** for real trips, stopping before any Tencent request. The built-in demo trip explicitly uses `MockRouteOptionService` with the fixed Guangzhou fixture. `RealRouteOptionService` remains available for a later, deliberate re-enable.

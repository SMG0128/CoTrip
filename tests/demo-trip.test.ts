...
// 内置示例行程（Demo Trip）测试：
// - 全局 Mock 模式已移除：auth / trip / routeOption 服务全部为真实实现，无 Mock fallback
...
export async function runDemoTripTests(): Promise<void> {
  ...
  // ---- 8. 全局 Mock 模式已移除：核心链路一律真实实现，无 fallback ----
  ...
  assert(routeOptionService instanceof RealRouteOptionService, '路线方案走 RealRouteOptionService');
